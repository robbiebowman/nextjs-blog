...
function buildPuzzlePool(clueWords, answerWords, options = {}) {
  ...
  for (const clue of clues) {
    ...
    for (let wordIndex = 0; wordIndex < acceptedWords.length; wordIndex++) {
      const word = acceptedWords[wordIndex]
    ...
    for (const [code, wordIndex] of matchesByPattern) {
      ...
      const answer = acceptedWords[wordIndex]
