...
export const getElementColor = (categories, rangeMinColour, rangeMaxColour, rangeMax, answerValue) => {
    if (answerValue === null) {
        return "#FFFFFF";
    }
    if (rangeMinColour && rangeMaxColour) {
        return interpolateColor(rangeMinColour, rangeMaxColour, answerValue / rangeMax) ?? "#FFFFFF";
    } else if (categories.length > 0) {
        return categories.find(cat => cat.name === answerValue)?.hexColour ?? "#FFFFFF";
    } else {
        return "#FFFFFF";
    }
};
